...
export default function FinanceApprovalForm() {
  const [form] = Form.useForm()
  const [loading, setLoading] = useState(true)
  const [submitting, setSubmitting] = useState(false)
  const [templateName, setTemplateName] = useState('财务提款审批单')
  ...
  useEffect(() => {
    const loadTemplate = async () => {
      try {
        const [tplRes, usersRes] = await Promise.all([getFormTemplate('finance_withdrawal'), getUsers({})])
        const tpl = tplRes || {}
        setTemplateName(tpl.name || '财务提款审批单')
